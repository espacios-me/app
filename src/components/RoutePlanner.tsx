...
import { MapPin, Navigation, Loader2, Footprints, Car, CloudRain, Sparkles, ScrollText, Sword, TrendingUp } from 'lucide-react';
...
const STYLES: { id: StoryStyle; label: string; icon: React.ElementType; desc: string }[] = [
    { id: 'GROWTH', label: 'Growth & Automation', icon: TrendingUp, desc: 'Turn your journey into a path of business success.' },
...
const RoutePlanner: React.FC<Props> = ({ onRouteFound, appState, externalError }) => {
  ...
  const [selectedStyle, setSelectedStyle] = useState<StoryStyle>('GROWTH');
    ...
    <div className={`transition-all duration-700 ${isLocked ? 'opacity-50 pointer-events-none grayscale' : ''}`}>
      <div className="space-y-8 bg-white/80 backdrop-blur-lg p-8 md:p-10 rounded-[2rem] shadow-2xl shadow-stone-200/50 border border-white/50">
        <div className="space-y-1">
            <h2 className="text-2xl font-serif text-editorial-900">Plan Your Growth Journey</h2>
            <p className="text-stone-500">Search locations and choose your narrative style.</p>
        </div>
        ...
        >
          {isLoading ? (
            <>
              <Loader2 className="animate-spin" /> Calculating Path...
            </>
          ) : (
            <>
               <Sparkles size={20} className="animate-subtle-pulse" />
               Generate Your Growth Story
            </>
          )}
        </button>
      </div>
