...
export async function cloudflarePage(accountId, token) {
    if (!accountId) {
        return renderShell("cloudflare", `<h1>Cloudflare summary</h1><p class="muted">Set CLOUDFLARE_ACCOUNT_ID for live read-only summaries.</p>`);
    }
    ...
    try {
        ...
        const workerRows = workers
            ...
            .join("");
        return renderShell("cloudflare", `<h1>Cloudflare summary</h1>
       ...
       <article class="card">
        ...
        return renderShell("cloudflare", `<h1>Cloudflare summary</h1><p class="muted">Unable to load Cloudflare data: ${error.message}</p>`);
    }
}
