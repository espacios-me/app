...
export function renderShell(active, content) {
    const is = (route) => (route === active ? "nav-link active" : "nav-link");
    return `<!doctype html>
  <html lang="en">
  <head>
    <meta charset="utf-8" />
...
export function renderCards(cards) {
    return `<section class="grid">${cards
        .map((card) => `<article class="card"><div class="muted">${card.label}</div><div class="kpi">${card.value}</div><div class="muted">${card.detail ?? ""}</div></article>`)
        .join("")}</section>`;
}
