...
export async function githubPage(org, repo, token) {
    if (!org || !repo) {
        return renderShell("github", `<h1>GitHub summary</h1><p class="muted">Set GITHUB_ORG and GITHUB_REPO for live read-only summaries.</p>`);
    }
    ...
    try {
        ...
        const rows = repos
            .slice(0, 8)
            .map((item) => `<tr><td>${item.name}</td><td>${item.openIssuesCount}</td><td>${new Date(item.updatedAt).toISOString().slice(0, 10)}</td></tr>`)
            .join("");
        return renderShell("github", `<h1>GitHub summary</h1>
       ...
       <article class="card">
        ...
        return renderShell("github", `<h1>GitHub summary</h1><p class="muted">Unable to load GitHub data: ${error.message}</p>`);
    }
}
